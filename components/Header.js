...
export default function Header({ locale }) {
  ...
  const switchedPath = useMemo(() => {
    ...
    if (parts[0] === "uk" || parts[0] === "en") parts[0] = otherLocale;
    else parts.unshift(otherLocale);
    return `/${parts.join("/")}`;
  ...
  return (
    <header className={styles.header}>
      <div className={styles.inner}>
        <Link className={styles.logo} href={localeHref(locale, "/")} aria-label={t("brand.name")}>
          <Image src={siteConfig.logo} alt={t("brand.name")} width={2048} height={401} priority />
        </Link>

        <nav className={styles.desktopNav} aria-label="Primary navigation">
          {nav.map(([key, href]) => (
            <Link key={key} className={styles.navLink} href={localeHref(locale, href)}>{t(`nav.${key}`)}</Link>
          ))}
        </nav>
        ...
        <div className={styles.actions}>
          ...
          <Link className={styles.proposal} href={`${localeHref(locale, "/contact")}?topic=proposal`}>
            {t("nav.proposal")}<span aria-hidden="true">↗</span>
          </Link>
          <button
      ...
      <div id="mobile-menu" className={`${styles.mobilePanel} ${open ? styles.mobilePanelOpen : ""}`}>
        <div className={styles.mobileNav}>
          {nav.map(([key, href], index) => (
            <Link key={key} href={localeHref(locale, href)} className={styles.mobileLink} style={{ "--i": index }}>
              <span>{String(index + 1).padStart(2, "0")}</span>{t(`nav.${key}`)}
            </Link>
          ))}
          <Link className={styles.mobileProposal} href={`${localeHref(locale, "/contact")}?topic=proposal`}>
